...
### logistic regression - including class weighting, L2 regularization, and feature selection.
A simple model, logistic regression is quick to implement and will reveal more about the data, in particular how different features contribute to predictions [@MultinomialLogisticRegression18:24:37+00:00].

There are variations such as weighting and regularization[@jamesIntroductionStatisticalLearning2023, pp. 240-253] which will be implemented - the success of lack of success of these techniques will reveal characteristics about our data that will be valuable to inform the selection and implementation of other models [@WeightedLogisticRegression09:33:43+00:00]
...
Biological data is often naturally heirarchical, for example animals can be classified by deviding them into first kingdoms, then families of species, and finally species and sub-species [@caiESPRITForestParallelClustering2017].
...
### Gaussian mixture model clustering, including model seclection, regularization and dimensionality reduction using factor analysis.
...
All of the above modelling techniques were repeated on bootstrapped data with a resampling size of 10,000 data points. The results achieved were worse across the board, but some models were affected more than others.
...
# svm_bootstrap_conf_matrix <- confusionMatrix(bootstrap_predictions, bootstrap_test_data$label)
...
rf_orig_acc <- unname((rf_tuned_conf_df$overall)["Accuracy"])
...
rf_orig_d_f1 <- rf_tuned_conf_df$byClass[, "F1"]["Class: D"]




Model <- c("Logistic Regression", "Random Forest") # "Support Vector Machines", "Random Forest")
original_accuracy <- c(lr_orig_acc, rf_orig_acc) #, svm_orig_acc, rf_orig_acc)
original_d_f1 <- c(lr_orig_d_f1, rf_orig_d_f1) #, svm_orig_d_f1, rf_orig_d_f1)
bootstrap_accuracy <- c(lr_bootstrap_acc, rf_bootstrap_acc) #, svm_bootstrap_acc, rf_bootstrap_acc)
bootstrap_d_f1 <- c(lr_bootstrap_d_f1, rf_bootstrap_d_f1) #, svm_bootstrap_d_f1, rf_bootstrap_d_f1)
...
comparison_table <- comp_df %>% kable(
    caption = "Comparison of Model Performance on Bootstrapped Data",
    digits = 4,
    align = "lcccc",
    col.names = c("Model", "Accuracy - Original Data", "Class D F1 Score - Original Data", "Accuracy - Bootstrapped Data", "Class D F1 Score - Bootstrapped Data"),
    format = "latex",
    longtable = FALSE,
    booktabs = TRUE
  ) %>%
