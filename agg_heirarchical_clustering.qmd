...
```{r}
# Convert to numeric to get original row indices
leaf_indices <- as.numeric(leaf_labels)
# Get original class labels for these indices
...
# Color labels by their original class
pruned_ward_dend <- set(pruned_ward_dend, "labels_col", class_colors[class_labels])
# Make labels smaller for readability
pruned_ward_dend <- set(pruned_ward_dend, "labels_cex", 0.6)

# Plot the dendrogram
par(mar = c(5, 5, 5, 12)) # Increase right margin for legend
plot(pruned_ward_dend, 
     main = "Hierarchical Clustering - Ward Linkage",
     sub = paste0(n_samples, " randomly sampled observations"),
     horiz = TRUE)

# Add rectangles around clusters
rect.dendrogram(pruned_ward_dend, k = 4, border = "gray", lty = 2)
