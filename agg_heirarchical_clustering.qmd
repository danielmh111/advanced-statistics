...
hopkins_stat <- hopkins(binary_data, m = nrow(binary_data)/10)
hopkins_stat
```
```{r}
library(clustertend)
...
```
weird that this is so high - i dont really know how to interpret that...
why does it draw the line at 8 when the data point is still higher at 9 and 10?
